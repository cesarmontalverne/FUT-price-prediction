...
players <- players[players$league != "Icons",]
```
...
xbox_vec <- c("xbox_last","xbox_max", "xbox_min", "xbox_prp")
pc_vec <- c("pc_last","pc_max", "pc_min", "pc_prp")
line_ps4 <- line[,!names(line)%in%c(xbox_vec ,pc_vec,"ps4_prp")]
#line_xbox <- line[,!names(line)%in%c(ps4_vec,pc_vec)] #target would be mostly nulls
#line_pc <- line[,!names(line)%in%c(ps4_vec,xbox_vec)] #target would be mostly nulls
```
```{r}
```
#End of Peter's code